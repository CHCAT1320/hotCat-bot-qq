import { NCWebsocket, NCWebsocketOptions, Structs } from 'node-napcat-ts'
...
class botClient {
    ...
    constructor(config?: NCWebsocketOptions, debug?: boolean) {
        ...
        if (config) {
            this.api = new NCWebsocket(config, debug ?? false)
        }
    }
    setConfig(config: NCWebsocketOptions, debug?: boolean) {
        this.api = new NCWebsocket(config, debug ?? false)
    }
    async start() {
...
export const bot: botClient = new botClient(config)
await bot.start()
